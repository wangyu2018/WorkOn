...
/** 成就墙徽章稀有度配色（common 石板 / rare 蓝 / epic 紫 / legendary 金渐变） */
const RARITY_STYLE: Record<Achievement['rarity'], { border: string; bg: string; text: string; label: string }> = {
  common: { border: 'rgba(148,163,184,0.35)', bg: 'rgba(148,163,184,0.10)', text: '#CBD5E1', label: '普通' },
  rare: { border: 'rgba(59,130,246,0.45)', bg: 'rgba(59,130,246,0.12)', text: '#60A5FA', label: '稀有' },
  epic: { border: 'rgba(139,92,246,0.50)', bg: 'rgba(139,92,246,0.14)', text: '#A78BFA', label: '史诗' },
  legendary: {
    border: 'rgba(245,158,11,0.55)',
    bg: 'linear-gradient(135deg, rgba(245,158,11,0.28), rgba(236,72,153,0.18))',
    text: '#FBBF24',
    label: '传说'
  }
}

/** 成就墙：徽章 grid（已解锁彩色 / 未解锁灰锁）+ 最新解锁一行 */
function AchievementWall({ delay = 0 }: { delay?: number }) {
  ...
  useEffect(() => {
    const load = () =>
      window.api
        .listAchievements()
        .then((r) => setList(((r as Achievement[]) ?? []).slice()))
        .catch(() => undefined)
    void load()
    // 主进程解锁成就时推送事件，收到后重新拉取（含 unlockedAt）
    const off = window.api.onAchievementUnlocked?.(() => void load())
    return off
  }, [])

  const unlocked = list.filter((a) => a.unlocked)
  const latest = unlocked.reduce<Achievement | null>((acc, a) => (a.unlockedAt && (!acc || (acc.unlockedAt ?? 0) < a.unlockedAt) ? a : acc), null)

  return (
    <section className="glass-card hoverable anim-fade-up" style={{ animationDelay: `${delay}ms` }}>
      <div className="mb-3 flex items-center justify-between gap-2">
        <SectionTitle icon="flame" title="成就墙" />
        <span className="text-[11px] tabular-nums text-slate-500">
          已解锁 <span className="font-semibold text-neon-cyan">{unlocked.length}</span>/{list.length}
        </span>
      </div>
      {list.length > 0 ? (
        <>
          <div className="grid grid-cols-5 gap-2">
            {list.map((a) => {
              ...
              const tip = a.unlocked
                ? `${a.name}（${rs.label}）\n${a.description}\n解锁于 ${a.unlockedAt ? new Date(a.unlockedAt).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '—'}`
                : `${a.name}（${rs.label}）\n解锁条件：${a.description}`
              return (
                <div
                  key={a.id}
                  title={tip}
                  className={`flex h-12 w-12 items-center justify-center justify-self-center rounded-full border text-[14px] transition-all duration-200 ${
                    a.unlocked ? 'hover:-translate-y-0.5 hover:shadow-glow' : 'border-white/[0.07] bg-white/[0.03] opacity-40'
                  }`}
                  style={a.unlocked ? { borderColor: rs.border, background: rs.bg, color: rs.text, boxShadow: `0 0 8px ${rs.border}33` } : undefined}
                >
                  {a.unlocked ? <span className="max-w-full truncate px-1 font-medium">{a.name.slice(0, 1)}</span> : <Icon name="x" size={13} className="text-slate-500" />}
                </div>
              )
            })}
          </div>
          <div className="mt-3 border-t border-white/[0.06] pt-2.5 text-[11px] text-slate-500">
            {latest ? (
              <>
                最新解锁：<span className="font-medium text-neon-cyan">{latest.name}</span>
                <span className="ml-1.5 tabular-nums">
                  {latest.unlockedAt ? new Date(latest.unlockedAt).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''}
                </span>
              </>
            ) : (
              '暂无成就，继续努力'
        ...
        <div className="rounded-lg border border-dashed border-white/10 bg-white/[0.02] py-4 text-center text-[11px] text-slate-500">
          暂无成就，继续努力
        </div>
      )}
    </section>
